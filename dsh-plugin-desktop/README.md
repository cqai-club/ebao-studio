...
`dsh-plugin-desktop` runs DSH in Electron while remaining part of the ordinary Cordis composition. The installed application is named **易宝工坊**. The package provides the `dsh-plugin-desktop` executable and the `dsh-desktop` alias; the registered npm package name is the reliable `npx` entry.
...
This package is managed by the Yarn workspace at the repository root. The sibling `deepseek-harness/` checkout remains an independent upstream pnpm project and is not part of the Yarn workspace. Install and verify 易宝工坊 from the repository root:
...
Packaged macOS and Windows applications query the repository-owned manifest at `https://raw.githubusercontent.com/cqai-club/ebao-studio/master/release/desktop-version.json` 60 seconds after startup and every six hours after a completed check. Each no-cache request has a 15-second deadline, sends `X-DSH-Desktop-Channel: stable` together with the installed version, and shares one in-flight operation with the **Check for Updates…** tray command. Stable accepts only canonical release SemVer from a manifest explicitly marked `channel: "stable"` and never discovers a Beta response. Background failures and non-newer versions remain silent; a manual check always opens a native result dialog. A background discovery sends one system notification per version; clicking it focuses Desktop and invokes the same confirmation, recheck, and download flow as the tray command. Development, unpackaged, and Linux launches do not download an installer.

Choosing **Download** rechecks the manifest, then Electron Updater reads the matching GitHub Release `latest.yml` on Windows or `latest-mac.yml` on macOS. Its result must equal the manifest-confirmed version. Electron Updater verifies the release metadata SHA-512 while staging a complete NSIS installer or macOS ZIP in its private cache; differential and web-installer fallbacks are disabled. There is no save dialog, user-selected destination, opened DMG, or manual EXE handoff. A completed transfer shows **Restart and Update**. Only that explicit choice asks Electron to quit; the existing Cordis teardown runs, then Electron Updater's platform helper replaces the application and launches the new version. Check, metadata, transport, cancellation, and staging failures leave the current version usable and retryable. macOS updating requires the official Developer ID-signed and notarized ZIP; `SHA256SUMS` remains published for independent validation of the manual DMG/Windows artifacts.

Release operators must publish `eBao-Studio-<version>-universal.dmg`, `eBao-Studio-<version>-universal.zip`, `eBao-Studio-<version>-x64-Setup.exe`, `eBao-Studio-<version>-x64-Portable.zip`, `latest.yml`, `latest-mac.yml`, and matching `SHA256SUMS` under the `v<version>` GitHub prerelease tag before making that stable version discoverable in `release/desktop-version.json`. The workflow checks the exact asset set, SHA-256 checksums, and updater-metadata SHA-512 values before and after publication. Ordinary tag releases fail closed unless the configured Developer ID certificate and Apple notarization credentials produce a signed and notarized macOS updater artifact. A release operator can select `workflow_dispatch` mode `unsigned-test` only for an existing canonical tag; it labels the macOS assets as an unsigned test release, requires testers to manually download and pass Gatekeeper, and makes no production in-place-update claim. Missing, unavailable, mismatched, or invalid manifest values produce no Desktop prompt, and a missing or unverifiable updater asset makes the confirmed download fail safely.

On macOS and Windows, **Open DSH Terminal** opens a system terminal rooted at the active profile. During unpackaged development launches, the settings header also shows **Export Diagnostics**, **Open DSH Terminal**, and a restart menu with **Restart Desktop** and **Restart in Recovery Mode**. Packaged builds leave this settings-header action group disabled by default; equivalent diagnostic and recovery operations remain available from the tray. Every restart path opens a confirmation before orderly Cordis shutdown and Electron relaunch. The terminal welcome text identifies the application version, active profile, profile directory, and DSH home, then lists configuration and plugin-management commands. Inside this terminal, bare `dsh`, `dsh --dump-config`, and plugin subcommands without a profile selection default to that active profile; an explicit `--profile` and the upstream `web` alias keep their original meaning. 易宝工坊 generates private per-profile `dsh`, `pnpm`, and `node` shims under its user-data directory, sets `DSH_HOME`, uses the active profile as the working directory, and prepends the shim directory only to that terminal's `PATH`. A later profile switch therefore does not change commands in an already open terminal. It does not edit the global environment or shell startup files. The macOS launcher preserves the user's interactive zsh or bash setup before restoring the desktop-owned values. Windows selects PowerShell 7, Windows PowerShell, or Command Prompt in that order and opens it in a new Windows Terminal window; when `wt.exe` is unavailable, a private `cmd start` broker creates a visible console instead. Synchronous launch failures and unsuccessful broker exits use the Desktop dialog surface. Linux does not compose the terminal command.

Desktop confirmations, warnings, errors, and results use one shadcn-backed `DesktopDialogWindow`. Each dialog is a separate sandboxed modal `BrowserWindow`, parented to the active Desktop window when available; it is not a component or portal inside the official Web page. A parented action-only dialog has no caption controls and therefore renders no empty frame; a standalone dialog with native traffic lights or caption controls uses the shared empty 36-pixel utility frame. Escape or an available window close maps to the bounded cancel action, and only a one-shot local result reaches the main process. Operating-system file-open pickers remain native because they select system paths rather than present a Desktop operation.
...
After a healthy startup, an unexpected interface-process exit (including out-of-memory) silently reloads the existing window without restarting the Host, opening a prompt, or revealing a hidden window. Recovery succeeds only after both page loading and the client Loader's healthy report; an unresponsive recovery attempt times out after 30 seconds. There are at most three automatic attempts, with no initial delay and subsequent delays of one and three seconds. The retry budget resets only after a recovered client remains healthy for one minute. Repeated failures pause automatic recovery and open a system-native fallback prompt: **Try recovery again** authorizes another bounded cycle, while **Not now** leaves the background service running. Choose **Open 易宝工坊** from the tray to return to that prompt, or **Export Diagnostics…** to investigate. Reloading can briefly interrupt the display and lose unsent input; this recovery does not fix the underlying crash or memory growth. Startup failures retain their existing recovery flow; intentional renderer termination and application shutdown do not initiate automatic recovery.

易宝工坊 writes UTF-8 logs under Electron's user-data directory: `%APPDATA%\易宝工坊\logs` on Windows and `~/Library/Application Support/易宝工坊/logs` on macOS. Full logs use `dsh-YYYY-MM-DD.log`; warnings and errors are also written to `dsh-YYYY-MM-DD.error.log`. Files rotate at 10 MiB, files older than seven days are removed at startup, and the directory is kept below 200 MiB. The `dsh-desktop.logLevel` setting controls verbosity and defaults to `info`.
...
- Adding or removing a profile bundle requires restarting 易宝工坊; the launcher does not watch profile manifests. Selecting another profile from the tray performs that restart automatically.
...
- The stable update handoff verifies Electron Updater SHA-512 metadata and uses platform-managed replacement after **Restart and Update**. macOS production updates additionally require Developer ID signing and notarization; the local Windows `dist:win` artifact remains unsigned and can trigger SmartScreen. SHA-256 and SHA-512 do not replace platform publisher identity, Authenticode, SmartScreen reputation, or native upgrade testing.
...
- This project pins both the published DSH `0.1.5-rc.1` family and the corresponding official `deepseek-harness/` release source. Product builds consume the checked-in official-profile runtime tarballs recorded in `upstream.json`, rather than linking the source checkout.
- DSH `0.1.5-rc.1` migrates supported historical sessions to V3 while preserving the original logs. Sessions written after upgrading cannot be read by the previous `0.1.2-rc.1` runtime.
