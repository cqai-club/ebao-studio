...
function requestedArchitectures(
  ...
  // The CLI retains architecture flags even when NoOpTarget is omitted.
  // Without flags, match electron-builder's configured or process default.
  if (targetNames.size === 0) {
    const fromCli = cliArchitectures(argv, description)
    return fromCli.length > 0
      ? fromCli
      : [architectureNumber(result.configuration[key]?.defaultArch ?? process.arch, description)]
  }
