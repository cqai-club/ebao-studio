...
export function buildWindowsNsisAb(options: WindowsNsisAbBuildOptions): WindowsNsisAbManifest {
  ...
  options.run(
    ...
    [
      ...
      '--unsafe-paths',
      // The patch already uses package-relative paths, so -C anchors it at the
      // isolated app-builder-lib copy without platform-sensitive path rewriting.
  ...
  const installerName = `易宝工坊-${version}-x64-Setup.exe`
