  ...
  const options: WindowsNsisAbBuildOptions = {
    ...
    run: (command, args, cwd, env) => {
      ...
      if (args.includes('--dir')) {
        ...
        writeFileSync(join(app, '易宝工坊.exe'), pe())
      ...
      writeFileSync(join(output, '易宝工坊-9.8.7-x64-Setup.exe'), pe())
...
describe('Windows NSIS A/B packaging', () => {
  ...
  it('inspects a readable ASAR and its physical resources', () => {
    ...
    writeFileSync(join(installRoot, '易宝工坊.exe'), pe())
    ...
    writeFileSync(join(installRoot, 'Uninstall 易宝工坊.exe'), pe())
  ...
  it('reports a corrupt app.asar without throwing away the resource identity', () => {
    ...
    writeFileSync(join(root, '易宝工坊.exe'), pe())
