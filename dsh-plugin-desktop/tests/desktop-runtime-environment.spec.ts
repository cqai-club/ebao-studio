...
function options(
  ...
  return {
    ...
    appExecutable: platform === 'win32'
      ? 'C:\\Program Files\\DSH 100% Desktop\\易宝工坊.exe'
      : "/Applications/DSH O'Brien.app/Contents/MacOS/易宝工坊",
    pnpmBinPath: platform === 'win32'
      ? 'C:\\Program Files\\易宝工坊\\resources\\app.asar.unpacked\\node_modules\\pnpm\\bin\\pnpm.mjs'
...
describe('desktop Host dsh runtime', () => {
  it('content-addresses Windows DSH shims and reuses an exact generation', () => {
    ...
    const runtimeOptions = {
      ...
      appExecutable: 'C:\\Program Files\\易宝工坊\\易宝工坊.exe',
      dshBootstrapPath: 'C:\\Program Files\\易宝工坊\\resources\\app.asar\\desktop-cli.js',
      profileName: 'web',
...
describe("RunAsNode scope", () => {
  ...
  it("scopes the flag to the runner child in every active runtime patch", () => {
    const upstream = JSON.parse(readFileSync(new URL('../../upstream.json', import.meta.url), 'utf8')) as {
      channels: Record<string, { runtimePackageVersion: string }>
    }
    for (const channel of new Set(Object.values(upstream.channels).map(value => value.runtimePackageVersion))) {
