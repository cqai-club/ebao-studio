...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    title: '易宝工坊 Recovery Assistant',
    fallbackBody: 'The recovery information could not be read. Quit and start 易宝工坊 again.',
    ...
    confirmRestoreDefaultDirectoryBody: '易宝工坊 will use the default data directory for this system after restart. The current data directory will not be deleted.',
    ...
    confirmCreateDefaultDirectoryBody: '易宝工坊 will create a new environment at the default path and use it after restart. The current data directory will not be deleted.',
    confirmCreateDefaultDirectoryAction: 'Create and restart',
    confirmFactoryReset: 'Factory reset 易宝工坊?',
    confirmFactoryResetMessage: 'Remove current DSH data and initialize a new environment?',
    confirmFactoryResetBody: currentDirectory => `The following directory will be moved to the operating-system Trash or Recycle Bin:\n\n${currentDirectory}\n\nProfiles, plugins, settings, credentials, sessions, attachments, and workspace records stored there will be removed from 易宝工坊. Project files outside this directory are not deleted. 易宝工坊 will restart and create a clean default Profile.`,
  ...
  zh: {
    title: '易宝工坊 恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动 易宝工坊。',
    reason: '进入恢复模式的原因',
    ...
    confirmRestoreDefaultDirectoryBody: '易宝工坊 将改为使用当前系统的默认数据目录。当前数据目录不会被删除。',
    ...
    confirmCreateDefaultDirectoryBody: '易宝工坊 将在默认路径创建一个全新的环境并重启。当前数据目录不会被删除。',
    confirmCreateDefaultDirectoryAction: '新建并重启',
    confirmFactoryReset: '恢复 易宝工坊 出厂设置？',
    confirmFactoryResetMessage: '移除当前 DSH 数据并重新初始化？',
    confirmFactoryResetBody: currentDirectory => `以下目录将被移入系统废纸篓或回收站：\n\n${currentDirectory}\n\n其中保存的 Profile、插件、设置、凭据、会话、附件和工作区记录都会从 易宝工坊 中移除；不会删除此目录以外的项目文件。随后 易宝工坊 会重启并创建干净的默认 Profile。`,
