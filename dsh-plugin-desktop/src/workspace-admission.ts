...
export class ElectronWorkspaceAdmission {
  ...
  async validateDirectory(path: string): Promise<boolean> {
    ...
    if (decision.action === 'confirm') {
      const result = await this.options.showMessageBox({
        ...
        detail: zh
          ...
          : `Disconnecting the drive while 易宝工坊 is running can break commands or plugin operations. Keep it connected.\n\n${path}`,
    ...
    await this.options.showMessageBox({
      ...
      message: zh
        ? `${decision.concern.fileSystem ?? '当前文件系统'} 不能安全用作 易宝工坊 工作区。`
        : `${decision.concern.fileSystem ?? 'This filesystem'} cannot safely host a 易宝工坊 workspace.`,
      detail: zh
