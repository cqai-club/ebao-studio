...
const COPY: Record<DesktopLocale, DesktopNativeCopy> = {
  en: {
    ...
    hostStoppedInstructions: 'Open windows can no longer reach it, and no work is lost on disk. Restart 易宝工坊 to reconnect, or open DSH Terminal to inspect the logs first.',
  ...
  zh: {
    ...
    hostStoppedInstructions: '已打开的窗口无法再与它通信，磁盘上的内容不会丢失。重启 易宝工坊 即可重新连接；也可以先打开 DSH 终端查看日志。',
