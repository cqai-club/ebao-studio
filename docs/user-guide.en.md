...
Download the macOS or Windows installer from the product download page. 易宝工坊 includes Electron, Node, and its pinned DSH dependencies, so normal users do not need to install Node.js or pnpm separately.
...
Plugins are extensions that add capabilities to DSH, such as models, tools, interfaces, and workflows. 易宝工坊 uses the same plugin system as official Harness, so official plugins install and work directly; multiple plugins follow the same conventions and can be installed and used together.
...
In the terminal opened from the 易宝工坊 tray, bare `dsh` and plugin commands without `--profile` default to the active profile:
