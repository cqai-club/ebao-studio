...
Current prerelease artifacts support Windows x64 and universal macOS (Intel and Apple Silicon). `v0.0.5` is an explicitly **unsigned macOS test release** with CQAIclub sign-in and Publisher improvements; testers must deliberately download it and approve Gatekeeper manually before opening it. It is not a production macOS auto-update package. The earlier `v0.0.4` fixed the Electron startup-import failure in `v0.0.3`. Windows artifacts are not yet Authenticode-signed and may show a SmartScreen warning. There is currently no Linux installer.
