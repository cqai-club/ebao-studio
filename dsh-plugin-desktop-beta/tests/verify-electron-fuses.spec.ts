...
describe('final Electron fuse verification', () => {
  it('maps only requested platform and architecture keys to complete runtime contexts', () => {
    const expected = [
      ...
      join('/build', 'mac-universal', '易宝工坊 Beta.app', 'Contents', 'MacOS', '易宝工坊 Beta'),
      join('/build', 'win-unpacked', '易宝工坊 Beta.exe'),
      join('/build', 'win-arm64-unpacked', '易宝工坊 Beta.exe'),
    ].sort()
  ...
  it('ignores a stale sibling architecture from an earlier build', () => {
    const expected = join('/build', 'win-unpacked', '易宝工坊 Beta.exe')
    const stale = join('/build', 'win-arm64-unpacked', '易宝工坊 Beta.exe')
  ...
  it('fails when one requested architecture is missing even if a sibling exists', () => {
    const x64Executable = join('/build', 'win-unpacked', '易宝工坊 Beta.exe')
  ...
  it('checks every requested final executable after all artifact builds', async () => {
    const executables = [
      join('/build', 'mac-arm64', '易宝工坊 Beta.app', 'Contents', 'MacOS', '易宝工坊 Beta'),
      join('/build', 'mac', '易宝工坊 Beta.app', 'Contents', 'MacOS', '易宝工坊 Beta'),
    ]
    const events: string[] = []
  ...
  it('wraps an unreadable final executable with its resolved path', async () => {
    ...
    await expect(verifyElectronExecutableFuses('/build/易宝工坊 Beta.exe', read))
      .rejects.toThrow('/build/易宝工坊 Beta.exe')
  })
})
