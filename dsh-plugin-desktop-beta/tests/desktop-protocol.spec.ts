...
describe('Desktop protocol', () => {
  it('registers the packaged product scheme', () => {
    ...
    expect(registerDesktopProtocolClient({ setAsDefaultProtocolClient }, {
      execPath: 'C:\\Program Files\\易宝工坊 Beta\\易宝工坊 Beta.exe',
      argv: [],
    })).toBe(true)
    expect(DESKTOP_PROTOCOL_SCHEME).toBe('dsh-desktop-beta')
    expect(setAsDefaultProtocolClient).toHaveBeenCalledWith('dsh-desktop-beta')
  })

  it('registers the Electron development entry and accepts only the fixed callback', () => {
    const setAsDefaultProtocolClient = vi.fn(() => true)
    const entry = 'E:\\workspace\\project\\dsh-desktop\\dsh-plugin-desktop-beta\\lib\\main.js'
    ...
    expect(DESKTOP_LOGIN_COMPLETION_URL).toBe('dsh-desktop-beta://oauth/complete')
    expect(isDesktopLoginCompletionUrl(DESKTOP_LOGIN_COMPLETION_URL)).toBe(true)
    expect(isDesktopLoginCompletionUrl('dsh-desktop-beta://oauth/complete?code=secret')).toBe(false)
    expect(isDesktopLoginCompletionUrl('https://example.test/oauth/complete')).toBe(false)
    expect(DESKTOP_PAYMENT_RESULT_URL).toBe('dsh-desktop-beta://payment/result')
