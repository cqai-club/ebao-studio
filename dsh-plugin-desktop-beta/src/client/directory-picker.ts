...
export async function requestDesktopDirectory(
  ...
  if (!response.ok) throw new Error('易宝工坊 could not open the system folder picker')
  const value: unknown = await response.json()
  if (!isResponse(value)) throw new Error('易宝工坊 received an invalid response from the system folder picker')
  return value.path
}
...
export async function requestDesktopDirectoryValidation(
  ...
  if (!response.ok) throw new Error('易宝工坊 could not validate the selected workspace')
  const value: unknown = await response.json()
  if (!isValidationResponse(value)) throw new Error('易宝工坊 received an invalid workspace validation response')
