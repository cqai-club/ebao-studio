...
`dsh-plugin-desktop-beta` runs the Beta channel of 易宝工坊 in Electron while remaining part of the ordinary Cordis composition. The installed application is named **易宝工坊 Beta**. The package provides only the `dsh-plugin-desktop-beta` executable and the `dsh-desktop-beta` alias, so it does not conflict with the stable npm package or commands.

The stable `dsh-plugin-desktop` package and this Beta package can be installed on the same machine. They use separate application names, system identities, shortcuts, single-instance locks, and Electron user-data directories. Both intentionally use the same `DSH_HOME` (or the default `~/.dsh`), so profiles, settings, plugins, and sessions are shared. Simultaneous installation is supported; reliable simultaneous execution is not, because both applications can contend for shared profile files, ports, and dependency state. This release supplies in-app automatic-update artifacts only for stable; Beta retains compatible checking and manual-result behavior but does not download or replace itself.
...
Packaged Beta applications continue to query the repository-owned manifest at `https://raw.githubusercontent.com/cqai-club/ebao-studio/master/release/desktop-version.json` 60 seconds after startup and every six hours after a completed check. Each no-cache request has a 15-second deadline and identifies the Beta channel and installed version. This release deliberately publishes no Beta `latest*.yml` or updater artifacts: Beta checks are compatible with the shared lifecycle but an available Beta version is reported as unavailable for in-app installation. Stable releases use the stable edition and its explicit **Restart and Update** flow. Development, unpackaged, and Linux launches do not download an installer.

Beta does not expose an in-app download action in this release. Its shared lifecycle keeps manual checks, bounded requests, notification-action plumbing, version validation, and teardown behavior compatible with stable without claiming that unshipped Beta artifacts can be installed.

Release operators must not add a Beta version to the manifest unless a separately reviewed Beta release design, metadata, artifacts, signing policy, and verification gates are introduced. Stable `v<version>` releases remain the only official in-app update source.
...
易宝工坊 Beta writes UTF-8 logs under its separate Electron user-data directory: `%APPDATA%\易宝工坊 Beta\logs` on Windows and `~/Library/Application Support/易宝工坊 Beta/logs` on macOS. Full logs use `dsh-YYYY-MM-DD.log`; warnings and errors are also written to `dsh-YYYY-MM-DD.error.log`. Files rotate at 10 MiB, files older than seven days are removed at startup, and the directory is kept below 200 MiB. The `dsh-desktop.logLevel` setting controls verbosity and defaults to `info`.
...
- Beta has no matching GitHub Release updater assets and never stages or replaces itself in-app. Download a newer Beta manually. Stable-only Electron Updater staging is separately guarded by publisher identity, macOS Developer ID signing and notarization, Windows Authenticode/SmartScreen policy, and native upgrade testing.
