...
/** Next follows the upstream DSH release service; eBao Stable/Beta use their own feed. */
export const NEXT_VERSION_ENDPOINT = 'https://www.dshdesktop.cn/api/desktop/version'
...
export class NextUpdates {
  ...
  private async query(signal: AbortSignal): Promise<UpdateCheckResult | null> {
    ...
    return checkForDesktopUpdate({ currentVersion: this.options.version, channel: 'next', installationId,
      versionEndpoint: NEXT_VERSION_ENDPOINT,
