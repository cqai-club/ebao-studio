...
const COPY = {
  en: {
    ...
    unavailable: '易宝工坊 could not read this folder path',
  },
  zh: {
    ready: '松开以添加工作区',
    busy: '正在添加工作区…',
    invalid: '请只拖入一个文件夹',
    unavailable: '易宝工坊 无法读取这个文件夹路径',
  },
} as const
...
export async function adoptWorkspaceFolder(
  ...
  if (path.length === 0) throw new Error('易宝工坊 could not read this folder path')
  if (actions.validateDirectory !== undefined && !await actions.validateDirectory(path)) {
    throw new Error('易宝工坊 rejected this workspace location')
  }
