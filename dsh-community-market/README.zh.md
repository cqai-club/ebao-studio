...
DSH Community Market 是 [易宝工坊](../README.md) 内置的开放插件市场。它从用户选择的目录来源发现插件，并针对当前 Desktop Profile 执行简单的 npm package 操作。
...
来源请求仅允许 HTTPS，不携带凭据，具备边界限制，并防止不安全重定向和私有网络目标。来源故障不会改变用户选择，也不会阻止 易宝工坊 启动。
