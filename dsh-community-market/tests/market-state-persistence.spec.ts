...
import { DeferredMarketStateStore, MemoryMarketStateStore, type MarketCatalogCache } from '../src/catalog/state-store.js'
...
describe('community market durable state', () => {
  ...
  it('keeps an explicit source removal after a restart', async () => {
    const medium = new MemoryMedium()
    const first = await bootMarketState(medium)
    await first.state.setSources([source], { markDefaultSourceApplied: true })
    await first.state.setSources([])
    await first.dispose()

    const second = await bootMarketState(medium)
    expect(second.state.getSources()).toEqual([])
    expect(second.state.getDefaultSourceApplied()).toBe(true)
  })

  it('adopts an empty session registry whose default marker was applied before storage loaded', async () => {
    const pending = new DeferredMarketStateStore()
    await pending.setSources([], { markDefaultSourceApplied: true })
    const durable = new MemoryMarketStateStore()

    await pending.adopt(durable)

    expect(pending.getDefaultSourceApplied()).toBe(true)
    expect(durable.getDefaultSourceApplied()).toBe(true)
  })
  ...
  it('migrates an empty legacy registry with a completed product default', async () => {
    const home = await temporaryHome({
      'dsh-community-market': { sources: [], defaultSourceApplied: true },
    })
    const boot = await bootMarketState(new MemoryMedium(), home)
    expect(boot.state.getSources()).toEqual([])
    expect(boot.state.getDefaultSourceApplied()).toBe(true)
  })
