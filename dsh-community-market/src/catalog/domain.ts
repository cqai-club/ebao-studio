...
import type { DeferredMarketStateStore, MarketCatalogCache, MarketSourceSaveOptions, MarketStateStore } from './state-store.js'
...
export interface MarketSourcesRecord {
  ...
  /** Durable one-time product-default marker, kept with the registry for atomic writes. */
...
const sourcesRecordSchema = z.object({
  ...
  defaultSourceApplied: z.boolean().optional(),
})
...
export class DomainMarketStateStore implements MarketStateStore {
  ...
  async setSources(records: readonly LocalSourceRecord[], options: MarketSourceSaveOptions = {}): Promise<void> {
    const defaultSourceApplied = this.getDefaultSourceApplied() || options.markDefaultSourceApplied === true
    await this.sources.put(MARKET_SOURCES_KEY, {
      records,
      ...(defaultSourceApplied ? { defaultSourceApplied: true } : {}),
    })
  }

  getDefaultSourceApplied(): boolean {
    return this.sources.get(MARKET_SOURCES_KEY)?.defaultSourceApplied === true
  }
...
export async function importLegacyMarketSettings(ctx: Context, store: DomainMarketStateStore): Promise<void> {
  ...
  if (legacy !== undefined && store.getSources().length === 0 && !store.getDefaultSourceApplied()) {
    await store.setSources(legacy.records, legacy.defaultSourceApplied ? { markDefaultSourceApplied: true } : {})
    ctx.logger.info(
