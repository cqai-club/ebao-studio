...
export interface MarketSourceSaveOptions {
  /** Record that product defaults were considered in the same registry write. */
...
export interface MarketStateStore {
  ...
  setSources(records: readonly LocalSourceRecord[], options?: MarketSourceSaveOptions): Promise<void>
  /** Prevent a removed product default from being seeded again. */
  getDefaultSourceApplied(): boolean
...
export class MemoryMarketStateStore implements MarketStateStore {
  ...
  private defaultSourceApplied = false
  ...
  constructor(seed?: {
    ...
    if (seed?.defaultSourceApplied === true) this.defaultSourceApplied = true
  ...
  async setSources(records: readonly LocalSourceRecord[], options: MarketSourceSaveOptions = {}): Promise<void> {
    this.sources = records
    if (options.markDefaultSourceApplied === true) this.defaultSourceApplied = true
  }

  getDefaultSourceApplied(): boolean {
    return this.defaultSourceApplied
  }
...
export class DeferredMarketStateStore implements MarketStateStore {
  ...
  async setSources(records: readonly LocalSourceRecord[], options: MarketSourceSaveOptions = {}): Promise<void> {
    await this.delegate.setSources(records, options)
  }

  getDefaultSourceApplied(): boolean {
    return this.delegate.getDefaultSourceApplied()
  }
  ...
  async adopt(store: MarketStateStore): Promise<void> {
    ...
    const pendingDefaultSourceApplied = this.delegate.getDefaultSourceApplied()
    const pendingCache = this.delegate.getCatalogCache()
    if (!this.durable && (pending.length > 0 || pendingDefaultSourceApplied)) {
      if (store.getSources().length === 0 && !store.getDefaultSourceApplied()) {
        await store.setSources(pending, pendingDefaultSourceApplied ? { markDefaultSourceApplied: true } : {})
  ...
  release(): void {
    ...
    this.delegate = new MemoryMarketStateStore({
      ...
      defaultSourceApplied: this.delegate.getDefaultSourceApplied(),
