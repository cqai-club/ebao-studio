...
describe('unified publisher client contract', () => {
  it('registers one panel with internal navigation', () => {
    ...
    expect(entry).toContain("useState<PublisherTab>('publish')")
    expect(entry).toContain('className="pub-page-nav"')
    expect(entry.indexOf('className="pub-page-nav"')).toBeLessThan(entry.indexOf('id="pub-panel-publish"'))
    expect(entry).toContain('>发布历史</button>')
    expect(entry).toContain('>平台账号管理</button>')
    expect(entry).toContain('>返回发布内容</button>')
    expect(entry).toContain("{renderTypePanel('article')}")
    expect(entry).toContain("{renderTypePanel('image-note')}")
    expect(entry).toContain("{renderTypePanel('video')}")
    expect(entry).toContain('<DraftGallery contentType={type}')
    expect(entry).toContain('<ContentEditor contentType={type} active={active} selectedContentId={editingId}')
