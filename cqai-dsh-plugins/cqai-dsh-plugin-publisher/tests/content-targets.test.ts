...
describe('article and image-note account choices', () => {
  it('keeps adapter accounts selectable while Worker capability data loads', () => {
    const accounts = [account('juejin-1', 'juejin'), account('blbl-1', 'blbl'), account('xhs-1', 'xhs'), account('tt-1', 'tt'), account('bjh-1', 'bjh'), account('wxmp-1', 'wxmp')]
    ...
    expect(selectedContentAccounts('article', accounts, { tt: 'tt-1', bjh: 'bjh-1', wxmp: 'wxmp-1' })).toEqual(accounts.slice(3))
