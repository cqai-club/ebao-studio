...
- **文章和图文**：多份本地草稿、Markdown 编辑与简易安全预览、`.md/.txt` 导入、图片素材。文章首张图片自动作为封面，删除封面后自动改用下一张；图文图片支持拖动或按钮排序。掘金/B站专栏、小红书图文账号可先选择并准备草稿，账号选择不代表该平台已开放提交；页面按所选平台提示标题、素材数量和必填字段限制，平台通过真实验收后才由 Worker 能力矩阵开放提交。
...
- 当前文章适配器目标为掘金、B站专栏，图文目标为小红书；它们只可在受控实验配置下测试，默认安装包不展示为已可发布。文章正文插图尚未通过平台验收，简易 Markdown 预览也不等同于平台最终排版。
