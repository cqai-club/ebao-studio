...
预览中的“发布”只把当前草稿带到多平台发布页。发布页按准确的草稿 ID 打开，左侧可切换预览与编辑，右侧由用户选择平台账号及“立即发布／转存草稿”；只有“检查并提交”和最终确认后才进入本机队列。对话与发布页共用草稿修订号，发生跨页面修改冲突时需重新读取，不能静默覆盖。首期对话创作覆盖文章和图文，视频沿用原发布页流程。
