...
文章和图文也可以从原生 Agent 对话开始。每段对话关联一份主草稿：与 Agent 讨论出明确的新版本后，Agent 将标题、正文、摘要和标签写入本地草稿；用户在对话中上传或通过 e图宝生成并选定的图片，可以由 Agent 加入同一草稿，也可通过对话更换、移除图片及调整封面。图片引用必须来自当前会话。对话导入文章图片时请选 JPEG 或 PNG；图文仍可用 WebP。工具不会向发布 Worker 提交任务。右侧“内容预览”与原有“文件”并列，预览默认移动端宽度，可切换 PC 宽度；图片素材在预览内单独列出。会话与草稿的关联保存在本机，重新打开后仍可继续。
