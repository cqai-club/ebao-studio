...
- **文章和图文**：多份本地草稿、Markdown 编辑与简易安全预览、`.md/.txt` 导入、图片素材。文章首张图片自动作为封面，删除封面后自动改用下一张；素材列表可将图片以 `ebao-asset://<UUID>` 插入正文，封面也可兼作插图。图文图片支持拖动或按钮排序。已有适配器的掘金/B站专栏、头条、百家号、微信公众号文章和小红书图文均开放草稿与立即发布；页面按所选平台提示标题、素材数量和必填字段限制。未实现的内容类型组合仍不可提交。

## 从 Agent 对话创作

文章和图文也可以从原生 Agent 对话开始。每段对话关联一份主草稿：与 Agent 讨论出明确的新版本后，Agent 将标题、正文、摘要和标签写入本地草稿；Agent 生成标题时允许普通句读标点，拒绝话题符号、Markdown 符号、表情和换行。用户在对话中上传或通过 e图宝生成并选定的图片，可以由 Agent 加入同一草稿，也可通过对话更换、移除图片及调整封面。图片引用必须来自当前会话。对话导入文章图片时请选 JPEG 或 PNG；图文仍可用 WebP。工具不会向发布 Worker 提交任务。右侧“内容预览”与原有“文件”并列，预览默认移动端宽度，可切换 PC 宽度；图片素材在预览内单独列出。会话与草稿的关联保存在本机，重新打开后仍可继续。
...
- 预览展示的是各平台**实际选用的文案和图片组合**。微信公众号预览按提交时的 Markdown 规则展示正文，并把封面和摘要放在独立字段区；未插入正文的素材不会作为正文图片展示。公众号的 Markdown 在 Worker 内转换为带内联样式的 HTML，再走官方草稿接口；其他平台沿用现有 Markdown、网页编辑器或图文输入格式。页面预览仍是近似排版，最终样式须在各平台后台核对。
...
- 文章/图文能力按用户要求全部开放已有适配器，但尚未完成全部真实平台验收；页面变化、权限和风控可能使任务失败或结果不明确，须到账号后台确认。头条和百家号支持正文插图转换与同账号 session 内上传，标签写入未验收，提交时跳过这些平台的文章标签；草稿标签保留供其他平台使用；掘金、B站专栏仍只支持单张封面。简易 Markdown 预览不等同于平台最终排版。
