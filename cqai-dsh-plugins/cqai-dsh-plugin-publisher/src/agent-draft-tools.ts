...
import { ARTICLE_THEMES, CREATIVE_STATEMENTS, PLATFORMS, type Platform } from './protocol.ts'
...
export const AGENT_PUBLISHER_GUIDANCE = [
  ...
  '文章草稿默认使用 editorial 排版主题；用户明确偏好简洁传统文章样式时可传 article_theme=classic，也可按其偏好选择 editorial、orangeheart、lapis、purple。后三种是参考文颜主题视觉语言的适配排版。只有文章使用排版主题，图文不使用。',
  '头条文章当前不录入独立摘要，提交时会自动跳过该字段。不要为了头条清空主草稿摘要；它仍可供其他平台使用。',
  '文章正文形成新版本时，组织清晰的小标题、短段落、列表和图片说明等语义结构，避免整篇连续纯文本；各平台最终视觉排版可能不同。不要自动创建用户未确认的平台版本。',
  ...
  const disposers = [
    ...
    ctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        article_theme: { type: 'string', enum: [...ARTICLE_THEMES], description: 'Article-only layout theme: editorial by default; classic, orangeheart, lapis, or purple when the user prefers those styles.' },
      ...
      async execute(args, exec) {
        ...
        return toolSnapshot(saveSessionDraft(sessionIdOf(exec.agent), {
          ...
          ...(args.article_theme === undefined ? {} : { articleTheme: args.article_theme }),
