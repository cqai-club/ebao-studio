...
export const AGENT_PUBLISHER_GUIDANCE = [
  ...
  '文章草稿默认使用 editorial 排版主题；用户明确偏好简洁传统文章样式时可在 publisher_save_draft 传 article_theme=classic，切回新版文章样式可传 editorial。图文不使用文章排版主题。',
  ...
  const disposers = [
    ...
    ctx.tools.register(defineTool({
      ...
      parameters: {
        ...
        article_theme: { type: 'string', enum: [...ARTICLE_THEMES], description: 'Article-only layout theme: editorial by default for a new article; classic for a traditional layout.' },
