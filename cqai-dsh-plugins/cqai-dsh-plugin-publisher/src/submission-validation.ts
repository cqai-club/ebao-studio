...
export function contentSubmissionError(
  ...
  if (content.contentType === 'article' && accounts.some(account => account.platform === 'wxmp') && !content.coverAssetId) return '微信公众号文章必须选择封面图片'
  if (content.contentType === 'article' && accounts.some(account => account.platform === 'wxmp')
    && content.assets.some(asset => asset.mime === 'image/webp')) return '已有 WebP 素材请重新上传，上传时会自动转为 JPEG；再删除旧素材并重新选择封面或插入正文'
  if (content.contentType === 'article' && accounts.some(account => account.platform === 'wxmp')) {
    if (content.title.length > 64) return '微信公众号文章标题不能超过 64 字'
    if (content.summary.length > 120) return '微信公众号文章摘要不能超过 120 字'
    ...
    try {
      const used = new Set(articleAssetIds(content))
      if (content.assets.some(asset => used.has(asset.id) && asset.bytes >= 1024 * 1024)) return '微信公众号正文图片必须小于 1MB'
  ...
  if (content.contentType === 'article' && accounts.some(account => account.platform === 'tt' || account.platform === 'bjh' || account.platform === 'wxmp')) {
