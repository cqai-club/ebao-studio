...
function Studio(){
  ...
  return <section className="sv"><style>{css}</style><div className="sv-wrap">
    ...
    </div></div><aside><div className="sv-card"><h2>字幕与编码</h2><div className="sv-field"><label>字幕引擎</label><select className="sv-input" value={settings.subtitle_provider} onChange={e=>setSettings({...settings,subtitle_provider:e.target.value as Settings['subtitle_provider']})}><option value="edge">Edge TTS 对齐</option><option value="whisper">Whisper 识别</option></select></div>
