...
function Studio(){
  ...
  return <section className="sv"><style>{css}</style><div className="sv-wrap">
    ...
    {tab==='assets'&&<div className="sv-grid"><div><div className="sv-card"><h2>03 · 素材与画面</h2>{choice('视频 / 图片来源','video_source',sources)}
      ...
      {!voicePreviewReady&&<p className="sv-help">可以先配置素材；开始制作前需完成配音试听。{voicePreviewIssue}</p>}
      ...
      {number('播放速度','video_clip_speed',.1,4,.1)}
       </div>{catalog?.video?.length?<div className="sv-card"><h2>CQAI Club 视频模型</h2>{catalog.video.map(m=><p className="sv-help" key={m.id}>{m.name} · {m.callable?'已声明视频生成接口':'账号目录未声明视频生成接口'}</p>)}<p className="sv-help">视频生成尚未接入当前素材制作流程。</p></div>:null}{voicePreviewReady&&<div className="sv-card"><h2>已确认的配音</h2><p className="sv-help">可边听旁白边选择画面，成片将复用这段音频。</p>{voicePreview!.artifacts.filter(a=>a.kind==='audio').map(a=><audio controls preload="metadata" key={a.file} src={url(voicePreview!,a.file)}/>)}</div>}</aside></div>}
