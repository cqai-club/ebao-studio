...
function Studio() {
  ...
  useEffect(() => {
    ...
    api<{connected: boolean}>('settings').then(v => setConnected(v.connected)).catch(() => {})
  ...
  async function create() {
    ...
    if (options.mode === 'digitalhuman' && !connected) {setError('请先连接自己的 InferFlow 账户'); return}
    ...
    try {
      ...
      if (options.mode === 'digitalhuman') {
        setBusy('估算 InferFlow 费用…'); await api(`quote?id=${next.id}`, {})
  ...
  return <section className="ejb" data-cqai-video-main=""><style>{css}</style><div className="ejb-wrap">
    ...
    <details className="ejb-card" open={!connected}><summary>个人 InferFlow 账户 · {connected ? '已连接' : '未连接'}</summary><label htmlFor="ejb-key">InferFlow API Key</label><input id="ejb-key" className="ejb-input" type="password" autoComplete="off" placeholder="粘贴你自己的 API Key" value={key} onChange={e => setKey(e.target.value)}/><p className="ejb-muted">密钥仅用于本次运行，退出后清除。U 盘内不保存密钥；数字人生成需要联网并消耗你的 InferFlow 积分。</p><div className="ejb-actions"><button className="ejb-secondary" disabled={!!busy || !key.trim()} onClick={async () => {setBusy('正在验证账户…'); setError(''); try {await api('settings', {key}); setKey(''); setConnected(true); setAccountMessage('账户和数字人服务已验证，可以估算费用。')} catch(e) {setError(e instanceof Error ? e.message : '连接失败')} finally {setBusy('')}}}>连接并验证</button>{connected && <button className="ejb-secondary" disabled={!!busy} onClick={async () => {try {await api('disconnect', {}); setConnected(false); setAccountMessage('')} catch(e) {setError(String(e))}}}>断开账户</button>}</div>{accountMessage && <p className="ejb-muted">{accountMessage}</p>}</details>
    ...
    {tab === 'new' ? <div className="ejb-grid"><div>
      <div className="ejb-card"><h2><span className="ejb-num">01</span>选择制作方式</h2><div className="ejb-modes">{([['video', '已有口播视频'], ['digitalhuman', '数字人口播'], ['plan', '仅生成动效方案']] as const).map(([mode, label]) => <button key={mode} className="ejb-mode" aria-pressed={options.mode === mode} onClick={() => {update('mode', mode); if (mode === 'digitalhuman') setOptions(prev => ({...prev, mode, optimize: false, covers: false}))}}>{label}</button>)}</div>
        <div className="ejb-muted">{options.mode === 'video' ? '保留原声，自动添加字幕、内容卡片和画中画动效。' : options.mode === 'digitalhuman' ? '使用你有权使用的形象照与参考录音，通过自己的 InferFlow 账户分段生成口播视频。先估算费用，确认后上传素材并使用 InferFlow 积分。' : '不生成视频，先整理文案并生成可编辑的 Remotion 动效组件包。'}</div>
    ...
    <button className="ejb-primary" disabled={!!busy || health?.python === false || jobs.some(j => j.status === 'running')} onClick={() => void create()}>{busy || (options.mode === 'plan' ? '生成动效方案' : options.mode === 'digitalhuman' ? '估算 InferFlow 费用' : '开始制作视频')}</button><p className="ejb-muted">任务保存在本机，失败后可继续。{options.mode !== 'plan' && '渲染期间请保持易宝工坊运行。'}</p></div></div> : <div className="ejb-grid"><div><div className="ejb-card"><h2>制作记录</h2>{jobs.length ? jobs.map(item => <button className="ejb-job" key={item.id} onClick={() => setSelected(item.id)}><span><strong>{item.options.title || '未命名视频'}</strong><small>{new Date(item.createdAt).toLocaleString()}</small></span><span className="ejb-muted">{statusText[item.status]}</span></button>) : <div className="ejb-empty">还没有制作记录<br/>从一段口播文案开始你的第一条视频。</div>}</div></div><div>{job ? <div className="ejb-card"><div className="ejb-summary"><h2>{job.options.title || '未命名视频'}</h2><span className="ejb-muted">{statusText[job.status]}</span></div>{stageSummary}{job.cloud && <p className="ejb-muted">预计消耗：{job.cloud.quote.displayAmount || (job.cloud.quote.amount + ' ' + job.cloud.quote.unit)}。按实际输出时长结算，最终以账户账单为准。{job.cloud.submissionStarted && '云端任务已提交，关闭页面或停止本地等待不会取消生成。'}</p>}{job.status === 'running' && <p className="ejb-muted">正在{job.stage ? LABELS[job.stage] : '准备素材'}…</p>}{job.error && <div className="ejb-error">{job.error}</div>}<div className="ejb-actions">{job.status === 'running' ? <button className="ejb-secondary" disabled={!!busy} onClick={() => void control('cancel')}>{job.cloud?.submissionStarted ? '停止本地等待' : '取消任务'}</button> : job.status !== 'completed' && <button className="ejb-primary" disabled={!!busy} onClick={() => void control('start')}>{job.options.mode === 'digitalhuman' && !job.cloud?.submissionStarted ? job.cloud && Date.parse(job.cloud.quote.expiresAt) > Date.now() ? '确认费用并生成' : '估算 InferFlow 费用' : '继续任务'}</button>}</div>
