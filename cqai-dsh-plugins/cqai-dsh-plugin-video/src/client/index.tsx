...
function Studio() {
  ...
  async function create() {
    ...
    try {
      ...
      setBusy('启动制作…'); await api(`start?id=${next.id}`, {}); await refresh(); setTab('history')
  ...
  async function control(action: 'start' | 'cancel') {
    ...
    try {await api(`${action}?id=${job.id}`, {}); await refresh()} catch (e) {setError(e instanceof Error ? e.message : '操作失败')} finally {setBusy('')}
  }
  ...
  return <section className="ejb" data-cqai-video-main=""><style>{css}</style><div className="ejb-wrap">
    ...
    {tab === 'new' ? <div className="ejb-grid"><div>
      <div className="ejb-card"><h2><span className="ejb-num">01</span>选择制作方式</h2><div className="ejb-modes">{([['video', '已有口播视频'], ['digitalhuman', '数字人口播'], ['plan', '仅生成动效方案']] as const).map(([mode, label]) => <button key={mode} className="ejb-mode" aria-pressed={options.mode === mode} onClick={() => update('mode', mode)}>{label}</button>)}</div>
        <div className="ejb-muted">{options.mode === 'video' ? '保留原声，自动添加字幕、内容卡片和画中画动效。' : options.mode === 'digitalhuman' ? '使用你有权使用的形象照与参考录音，通过 InferFlow 生成口播视频。启动任务后会调用云服务并消耗额度。' : '不生成视频，先整理文案并生成可编辑的 Remotion 动效组件包。'}</div>
      ...
      <label className="ejb-check"><input type="checkbox" checked={options.optimize} onChange={e => update('optimize', e.target.checked)}/><span>AI 优化文案与标题<div className="ejb-muted">调用已配置的 DeepSeek API；会发送文案并消耗额度。</div></span></label>
      <label className="ejb-check"><input type="checkbox" checked={options.covers} onChange={e => update('covers', e.target.checked)}/><span>生成三种尺寸的封面<div className="ejb-muted">调用已配置的通义万相 API，消耗图片生成额度。</div></span></label>
    ...
    <button className="ejb-primary" disabled={!!busy || health?.python === false || jobs.some(j => j.status === 'running')} onClick={() => void create()}>{busy || (options.mode === 'plan' ? '生成动效方案' : '开始制作视频')}</button><p className="ejb-muted">任务保存在本机，失败后可继续。{options.mode !== 'plan' && '渲染期间请保持易宝工坊运行。'}</p></div></div> : <div className="ejb-grid"><div><div className="ejb-card"><h2>制作记录</h2>{jobs.length ? jobs.map(item => <button className="ejb-job" key={item.id} onClick={() => setSelected(item.id)}><span><strong>{item.options.title || '未命名视频'}</strong><small>{new Date(item.createdAt).toLocaleString()}</small></span><span className="ejb-muted">{statusText[item.status]}</span></button>) : <div className="ejb-empty">还没有制作记录<br/>从一段口播文案开始你的第一条视频。</div>}</div></div><div>{job ? <div className="ejb-card"><div className="ejb-summary"><h2>{job.options.title || '未命名视频'}</h2><span className="ejb-muted">{statusText[job.status]}</span></div>{stageSummary}{job.status === 'running' && <p className="ejb-muted">正在{job.stage ? LABELS[job.stage] : '准备素材'}…</p>}{job.error && <div className="ejb-error">{job.error}</div>}<div className="ejb-actions">{job.status === 'running' ? <button className="ejb-secondary" disabled={!!busy} onClick={() => void control('cancel')}>取消任务</button> : job.status !== 'completed' && <button className="ejb-primary" disabled={!!busy} onClick={() => void control('start')}>继续任务</button>}</div>
