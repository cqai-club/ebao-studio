...
export interface UpstreamConfig {
  ...
  apiKey: string
  /**
   * Force the standard OpenAI image contract even when the model id resembles
   * a provider for which the standalone plugin has a native adapter.  CQAI's
   * Relay owns that provider translation and always receives the two OpenAI
   * image endpoints.
   */
  protocol?: 'auto' | 'openai'
  /**
   * Optional host-side transport.  The CQAI account service uses this seam to
   * attach its short-lived OAuth credential without ever exposing it to this
   * engine, the browser bundle, or a canvas skill.
   */
  request?: (path: `/v1/${string}`, init: RequestInit, signal?: AbortSignal) => Promise<Response>
}
...
/**
 * Send an image API request through the provider-owned transport when one is
 * present.  The path deliberately starts at `/v1`: this is the only surface
 * made available to the CQAI account bridge and prevents callers from using
 * the image engine as a general authenticated proxy.
 */
async function sendUpstream(
  ...
  if (upstream.request !== undefined) return upstream.request(path, init, signal)
  return fetch(`${baseUrl}${path.slice('/v1'.length)}`, { ...init, signal })
}
...
function effectiveParams(request: GenerateRequest, forceOpenAI = false): {
  model: string
  size?: string
  quality?: string
  detail?: string
  ...
  // host downloads them.
  if (!forceOpenAI && isGrokImagine(model)) {
    return {
      model,
  ...
  if (!forceOpenAI && isNanoBanana(model)) {
    return {
      model,
      ...request.size !== '' && request.size !== 'auto'
  ...
  if (!forceOpenAI && isSeedream(model)) {
    return {
      model,
  ...
  if (!forceOpenAI && isZhipuImage(model)) {
    return {
      model,
...
async function normalizeItem(
  ...
  try {
    try {
      ...
      return { b64: downloaded.buffer.toString('base64'), mime: downloaded.mime, revisedPrompt }
    } catch (error) {
...
async function requestOneImage(
  ...
  const forceOpenAI = upstream.protocol === 'openai'
  const headers: Record<string, string> = upstream.apiKey.trim() === ''
    ? {}
  ...
  if (request.mode === 'edit') {
    ...
    if (!forceOpenAI && isGrokImagine(params.model)) {
    ...
    } else if (!forceOpenAI && isNanoBanana(params.model)) {
      ...
      body = form
    } else if (!forceOpenAI && isSeedream(params.model)) {
  ...
  try {
    ...
    try {
      ...
      const endpoint = request.mode === 'edit' && (forceOpenAI || !isSeedream(params.model))
        ? '/images/edits'
        : '/images/generations'
      response = await sendUpstream(upstream, baseUrl, `/v1${endpoint}`, {
        method: 'POST',
        headers,
        body,
      }, budget.signal)
...
export async function generateImage(upstream: UpstreamConfig, request: GenerateRequest, options: { signal?: AbortSignal } = {}): Promise<GenerateResult> {
  ...
  if (upstream.apiKey.trim() === '' && upstream.request === undefined) throw new ImageGenError('api_key 未配置：请先在「设置 → 插件 → 可配置」中填写', 'config-missing')
  const forceOpenAI = upstream.protocol === 'openai'
  if (!forceOpenAI && isQwenImage(wireModel(request))) return generateQwenImage(baseUrl, upstream, request, options)
  if (!forceOpenAI && isMiniMaxImage(wireModel(request))) return generateMiniMaxImage(baseUrl, upstream, request, options)
  if (!forceOpenAI && request.mode === 'edit' && isZhipuImage(wireModel(request))) {
    throw new ImageGenError('智谱 GLM-Image 当前仅支持文生图，请切换到文生图模式或选择支持图生图的模型', 'edit-unsupported')
  }
  const params = effectiveParams(request, forceOpenAI)
