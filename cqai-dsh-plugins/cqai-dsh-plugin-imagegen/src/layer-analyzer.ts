...
import { chatComplete, stripReasoning, type ChatOptions, type PromptModelConfig } from './prompt-enhancer.ts'
 ...
 */
export async function analyzeLayersWithChat(
  complete: (options: ChatOptions) => Promise<string>,
  image: string,
): Promise<CanvasLayerPlan> {
...
/** Backward-compatible custom-channel entry point. */
export async function analyzeLayers(config: PromptModelConfig, image: string): Promise<CanvasLayerPlan> {
  if (config.apiUrl.trim() === '' || config.model.trim() === '') {
    throw new Error('图层拆分需要先配置一个支持视觉的聊天模型')
  }
  return analyzeLayersWithChat(options => chatComplete(config, options), image)
}
