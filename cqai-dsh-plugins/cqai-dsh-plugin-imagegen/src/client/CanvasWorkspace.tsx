...
interface CanvasWorkspaceProps {
  ...
  /** CQAI with several models and no account default must not pick the first. */
  requireExplicitImageModel: boolean
  connected: boolean
 ...
 * user touched.
 */
export function SkillConfigForm(props: {
  name: string
  config: CanvasSkillConfigView
  busy: boolean
  onSave: (name: string, values: Array<{ id: string; value: string }>) => Promise<CanvasSkillConfigSaveResult>
  onPreview: (name: string) => Promise<CanvasSkillConfigPreviewResult>
  onApply: (name: string, confirmationToken: string) => Promise<CanvasSkillConfigApplyResult>
}): React.JSX.Element {
  ...
  const [phase, setPhase] = useState<'idle' | 'saving' | 'previewing' | 'applying'>('idle')
  const [steps, setSteps] = useState<CanvasSkillConfigStep[] | null>(null)
  const [preview, setPreview] = useState<CanvasSkillConfigPreviewResult | null>(null)
  ...
  const save = async (apply: boolean): Promise<void> => {
    setPhase(apply ? 'previewing' : 'saving')
    setMessage(null)
    setSteps(null)
    try {
      ...
      const planned = await props.onPreview(props.name)
      setPreview(planned)
      setMessage(planned.ready ? tt('canvas.skills.configPreviewReady') : planned.message ?? tt('canvas.skills.configPreviewFailed'))
    } catch (caught) {
  ...
  const confirmApply = async (): Promise<void> => {
    ...
    setSteps(null)
    try {
      const applied = await props.onApply(props.name, token)
      setPreview(null)
    ...
    <div className={css.libraryRow}>
      ...
      {props.config.applicable
        ? <button
          ...
          >{phase === 'previewing' ? tt('canvas.skills.configPreviewing') : tt('canvas.skills.configSaveApply')}</button>
        : null}
    </div>
    {message === null ? null : <p className={css.skillHint}>{message}</p>}
    {preview === null ? null : <section className={css.skillConfigPreview} aria-label={tt('canvas.skills.configPreviewTitle')}>
      <strong>{tt('canvas.skills.configPreviewTitle')}</strong>
      <p className={css.skillConfigHint}>{tt('canvas.skills.configPreviewHint')}</p>
      <ul className={css.skillConfigSteps}>
        {preview.steps.map((step, index) => <li key={`${index}-${step.detail}`} data-status={step.status}>
          <span>{step.kind === 'command' ? '▶' : '▤'} {step.detail}</span>
          {step.content === undefined ? null : <code>{step.content}</code>}
          {step.issue === undefined ? null : <code>{step.issue}</code>}
        </li>)}
      </ul>
      <div className={css.libraryRow}>
        <button type="button" className={css.skillGhost} disabled={phase !== 'idle'} onClick={() => setPreview(null)}>{tt('canvas.skills.configPreviewCancel')}</button>
        <button
          type="button"
          className={css.skillPrimary}
          disabled={phase !== 'idle' || preview.confirmationToken === undefined}
          onClick={() => { void confirmApply() }}
        >{phase === 'applying' ? tt('canvas.skills.configApplying') : tt('canvas.skills.configConfirmApply')}</button>
      </div>
    </section>}
...
function SkillLibraryDialog(props: {
  ...
  onPreviewConfig: (name: string) => Promise<CanvasSkillConfigPreviewResult>
  onApplyConfig: (name: string, confirmationToken: string) => Promise<CanvasSkillConfigApplyResult>
  onClose: () => void
}): React.JSX.Element {
            ...
            {entries.map(entry => <div key={entry.name} className={css.libraryEntryGroup}>
              ...
              {entry.config !== undefined && configOpen === entry.name
                ? <SkillConfigForm
                    ...
                    onPreview={props.onPreviewConfig}
...
export function CanvasWorkspace(props: CanvasWorkspaceProps): React.JSX.Element {
  ...
  const [composerModel, setComposerModel] = useState(() => selectCanvasImageModel('', imageModels, requireExplicitImageModel))
  ...
  const composerModelSourceRef = useRef({ defaultChannelId, requireExplicitImageModel })

  documentRef.current = document
  selectedIdsRef.current = selectedIds

  // A provider/policy transition starts a fresh choice. Catalog refreshes on
  // the same provider retain an explicit valid choice, but never manufacture
  // one when CQAI has several models and no account default.
  const imageModelKey = imageModels.join('\u0000')
  useEffect(() => {
    const previous = composerModelSourceRef.current
    const sourceChanged = previous.defaultChannelId !== defaultChannelId
      || previous.requireExplicitImageModel !== requireExplicitImageModel
    composerModelSourceRef.current = { defaultChannelId, requireExplicitImageModel }
    setComposerModel(current => selectCanvasImageModel(
      sourceChanged ? '' : current,
      imageModels,
      requireExplicitImageModel,
    ))
  }, [defaultChannelId, imageModelKey, requireExplicitImageModel])
  ...
  /** Fetch the complete redacted plan before the user can authorize it. */
  const previewSkillValues = useCallback(async (name: string): Promise<CanvasSkillConfigPreviewResult> => {
    setLibraryBusy(true)
    try {
      return await api.canvasSkillConfigPreview({ name })
    } catch (caught) {
      return { ok: false, ready: false, steps: [], message: errorMessage(caught) }
    } finally {
      setLibraryBusy(false)
    }
  }, [api])

  /** Run a previously previewed declaration with its one-time confirmation. */
  const applySkillValues = useCallback(async (name: string, confirmationToken: string): Promise<CanvasSkillConfigApplyResult> => {
    setLibraryBusy(true)
    try {
      const result = await api.canvasSkillConfigApply({ name, confirmationToken })
      if (result.library !== undefined) setSkillLibrary(result.library)
      await loadSkillCatalog()
      return result
    } catch (caught) {
  ...
  const submitComposer = useCallback(async (target: CanvasNode | null): Promise<void> => {
    ...
    try {
      ...
      const model = selectCanvasImageModel(composerModel, imageModels, requireExplicitImageModel)
      if (model === '') {
        setError(tt(imageModels.length > 0 ? 'canvas.modelPlaceholder' : 'canvas.needModel'))
  ...
  }, [annotationPlanOf, api, canvasCenter, composerBusy, composerCount, composerModel, composerPrompt, composerQuality, composerSize, connected, defaultChannelId, imageModels, mutate, onOpenSettings, requireExplicitImageModel, upstreamNodes])
  ...
  const retryGeneration = useCallback(async (node: CanvasNode): Promise<void> => {
    ...
    const model = selectCanvasImageModel(metadata.model, imageModels, requireExplicitImageModel)
    if (model === '') { setError(tt(imageModels.length > 0 ? 'canvas.modelPlaceholder' : 'canvas.needModel')); return }
    ...
    return <div className={css.composer} data-canvas-no-zoom="" style={{ left: clampedX - 280, top }}>
      ...
      <div className={css.composerControls}>
        ...
        <button
          ...
          disabled={!connected || composerBusy
            || selectCanvasImageModel(composerModel, imageModels, requireExplicitImageModel) === ''
            || (composerPrompt.trim() === '' && composerTextCount === 0 && composerAnnotationTexts.length === 0)}
  ...
  return <section ref={rootRef} className={css.root} data-canvas-workspace="">
    ...
    {skillLibraryOpen ? <SkillLibraryDialog
      ...
      onPreviewConfig={previewSkillValues}
    ...
    {pickerOpen ? <ImagePicker
      ...
      requireExplicitImageModel={requireExplicitImageModel}
...
function ImagePicker(props: {
  ...
  const { api, history, gallery, imageModels, defaultChannelId, requireExplicitImageModel, canvasId, connected, onClose, onAssets } = props
  const [tab, setTab] = useState<'upload' | 'history' | 'gallery' | 'generate'>(props.initialTab ?? 'upload')
  const [selected, setSelected] = useState<string[]>([])
  const [dimensions, setDimensions] = useState<Record<string, { width: number; height: number }>>({})
  const [prompt, setPrompt] = useState('')
  const [model, setModel] = useState(() => selectCanvasImageModel('', imageModels, requireExplicitImageModel))
  const [size, setSize] = useState('auto')
  const [quality, setQuality] = useState('auto')
  const [busy, setBusy] = useState(false)
  const modelSourceRef = useRef({ defaultChannelId, requireExplicitImageModel })
  const imageModelKey = imageModels.join('\u0000')
  useEffect(() => {
    const previous = modelSourceRef.current
    const sourceChanged = previous.defaultChannelId !== defaultChannelId
      || previous.requireExplicitImageModel !== requireExplicitImageModel
    modelSourceRef.current = { defaultChannelId, requireExplicitImageModel }
    setModel(current => selectCanvasImageModel(
      sourceChanged ? '' : current,
      imageModels,
      requireExplicitImageModel,
    ))
  ...
  const generate = async (): Promise<void> => {
    ...
    const selectedModel = selectCanvasImageModel(model, imageModels, requireExplicitImageModel)
    if (selectedModel === '') return
    setBusy(true)
    try {
      const task = await api.taskSubmit({ mode: 'text', model: selectedModel, prompt: prompt.trim(), size, quality, n: 1, detail: '', ...(defaultChannelId === undefined ? {} : { channelId: defaultChannelId }), canvas: { canvasId } })
      props.onTask(task)
    } finally { setBusy(false) }
  }
  return <div className={css.modalBackdrop} role="dialog" aria-modal="true" data-canvas-no-zoom=""><section className={css.picker}>
    ...
    <div className={css.pickerBody}>
      ...
      {tab === 'generate' ? <div className={css.generateForm}><textarea value={prompt} onChange={event => setPrompt(event.target.value)} placeholder={tt('canvas.composerPlaceholder')} /><ComposerSelect value={model} options={[...(requireExplicitImageModel ? [{ value: '', label: tt('canvas.modelPlaceholder') }] : []), ...imageModels.map(item => ({ value: item, label: item }))]} ariaLabel={tt('canvas.model')} onChange={setModel} /><div className={css.inspectorRow}><ComposerSelect value={size} options={[{ value: 'auto', label: tt('canvas.sizeAuto') }, { value: '1:1', label: '1:1' }, { value: '3:4', label: '3:4' }, { value: '16:9', label: '16:9' }, { value: '9:16', label: '9:16' }]} ariaLabel={tt('canvas.size')} onChange={setSize} /><ComposerSelect value={quality} options={[{ value: 'auto', label: tt('canvas.qualityAuto') }, { value: '1k', label: '1K' }, { value: '2k', label: '2K' }, { value: '4k', label: '4K' }]} ariaLabel={tt('canvas.quality')} onChange={setQuality} /></div><button type="button" disabled={!connected || busy || prompt.trim() === '' || selectCanvasImageModel(model, imageModels, requireExplicitImageModel) === ''} onClick={() => { void generate() }}><ToolbarIcon name="sparkle" />{tt('canvas.generateAndAdd')}</button>{!connected ? <small>{tt('canvas.needApi')}</small> : null}</div> : null}
    </div>
  </section></div>
}
