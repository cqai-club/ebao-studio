...
export class ImageGenApi {
  /** Read the immutable CQAI Provider status and current image catalog. */
  async cqaiProvider(refresh = false): Promise<CqaiImageProviderView> {
    const response = await fetch(CQAI_IMAGE_PROVIDER_API, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ refresh }),
    })
    return (await readEnvelope<{ ok: true; provider: CqaiImageProviderView }>(response)).provider
  }
  ...
  /** Preview all redacted apply steps without executing them. */
  async canvasSkillConfigPreview(request: CanvasSkillConfigPreviewRequest): Promise<CanvasSkillConfigPreviewResult> {
    const response = await fetch(CANVAS_SKILL_API.configPreview, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ ...request, language: activeImageGenLanguage() }),
    })
    return await readEnvelope<CanvasSkillConfigPreviewResult>(response)
  }

  /** Run a skill declaration after a separately previewed confirmation. */
