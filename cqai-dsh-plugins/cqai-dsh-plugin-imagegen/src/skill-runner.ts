...
export interface SkillAgentBackend {
  ...
  create: (options: {
    ...
    systemPrompt: string
    signal?: AbortSignal
    /** Host-only run credential, installed as agent-scoped DSH_* variables. */
...
/** Safe compatibility facts resolved from one skill's explicit manifest. */
export interface SkillImageProviderBinding {
  protocol: 'openai-images-v1'
  /** Preferred model from a declared non-secret config field, when present. */
  model?: string
}

/** Values visible only inside one headless Agent's shell executions. */
export interface SkillImageProviderEnvironment {
  baseUrl: string
  apiKey: string
  model: string
}

/** Run lease issued by the Host bridge and revoked in the runner's finally. */
export interface SkillImageProviderLease extends SkillImageProviderEnvironment {
  revoke(): void
}

/** CQAI bridge seam. It never returns the OAuth token or Relay key. */
export interface SkillImageProviderBackend {
  issue(options: { model?: string; signal: AbortSignal }): Promise<SkillImageProviderLease>
}
...
export interface SkillConfigBackend {
  ...
  /** Explicit OpenAI Images v1 declaration, or undefined for an incompatible skill. */
  imageProvider: (name: string) => Promise<SkillImageProviderBinding | undefined>
}
...
export interface SkillRunnerBackend {
  ...
  /** Optional CQAI run-credential issuer for explicitly compatible skills. */
  imageProvider?: SkillImageProviderBackend
...
/** Agent-scoped variables; values never enter a prompt, task snapshot or log. */
export const DSH_IMAGEGEN_BASE_URL = 'DSH_IMAGEGEN_BASE_URL'
export const DSH_IMAGEGEN_API_KEY = 'DSH_IMAGEGEN_API_KEY'
export const DSH_IMAGEGEN_MODEL = 'DSH_IMAGEGEN_MODEL'
...
export class SkillRunner {
  ...
  private async runHeavy(
    ...
    let imageProvider: SkillImageProviderLease | undefined
    try {
      const declaredProvider = await this.backend.skillConfig?.imageProvider(skillName)
      const requestedProvider = request.params?.imageProvider?.trim() ?? ''
      const requestedModel = request.params?.imageModel?.trim() ?? ''
      const explicitlyRequested = requestedProvider !== '' || requestedModel !== ''
      if (declaredProvider === undefined && explicitlyRequested) {
        throw new Error('该技能未声明兼容 OpenAI Images v1，不能注入图像 Provider。')
      }
      if (declaredProvider !== undefined) {
        if (requestedProvider !== '' && requestedProvider !== 'cqai') {
          throw new Error(`画布技能的临时图像 Provider 只支持 cqai，不能使用「${requestedProvider}」。`)
        }
        if (this.backend.imageProvider === undefined) {
          throw new Error('CQAI 画布技能图像 Provider 当前不可用。')
        }
        // Canvas node metadata describes how the source image was produced;
        // it is not an instruction to reuse that (possibly third-party) model.
        // Only an explicit per-run override or the declaration's safe model
        // field may override the CQAI account default.
        const preferredModel = requestedModel || declaredProvider.model
        imageProvider = await this.backend.imageProvider.issue({
          ...preferredModel === undefined || preferredModel === '' ? {} : { model: preferredModel },
          signal: controller.signal,
        })
      }
      const systemPrompt = this.heavySystemPrompt(
        skill,
        body,
        fileLines,
        outputDir,
  ...
  private heavySystemPrompt(
    ...
    return [
      ...
      ...imageProviderAvailable ? [
        '',
        '### Run-scoped CQAI image provider',
        `This skill explicitly supports OpenAI Images v1. Before invoking its image backend, configure it from ${DSH_IMAGEGEN_BASE_URL}, ${DSH_IMAGEGEN_API_KEY}, and ${DSH_IMAGEGEN_MODEL}. Read those values only through the shell environment; never print, log, persist, or include them in a message. Do not use a saved or third-party image credential as fallback. If the skill cannot use this OpenAI Images v1 endpoint, stop and report that incompatibility.`,
      ] : [],
      '',
