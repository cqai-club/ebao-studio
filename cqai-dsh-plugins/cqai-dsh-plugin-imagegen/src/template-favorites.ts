...
const favoritesPath = (): string => path.join(imageDataRoot(), 'templates', 'favorites.json')
...
function syncMemoRoot(): void {
  const root = imageDataRoot()
  if (memoRoot === root) return
  memoRoot = root
  memo = undefined
}
...
export async function listTemplateFavorites(): Promise<TemplateFavorite[]> {
  ...
  try {
    const parsed: unknown = JSON.parse(await fs.readFile(favoritesPath(), 'utf8'))
