...
/** Safe status/model projection for the immutable CQAI image Provider. */
export const CQAI_IMAGE_PROVIDER_API = '/api/dsh-imagegen/cqai/provider' as const

/** Browser-safe CQAI state; credentials are intentionally absent. */
export interface CqaiImageProviderView {
  provider: 'cqai'
  immutable: true
  state: 'signed-out' | 'authorizing' | 'signed-in' | 'reauth-required' | 'error'
  models: ModelMapping[]
  defaultModel?: string
  stale?: boolean
  warning?: string
...
export const CANVAS_SKILL_API = {
  ...
  /** Build a redacted, non-executing apply plan and one-time confirmation. */
  configPreview: '/api/dsh-imagegen/canvas/skills/config/preview',
  /** Run a previously previewed declaration's `apply` steps. */
...
export interface CanvasSkillRunRequest {
   ...
   * Heavy skills reserve `imageProvider` (`cqai`) and `imageModel` for an
   * explicit run-scoped OpenAI Images v1 request.  These are selection hints
   * only; the temporary credential never enters this browser-facing payload.
   */
...
export interface CanvasSkillConfigView {
  ...
  applicable: boolean
  /** Explicit image API compatibility declared by this skill (never a key). */
  imageProvider?: { protocol: 'openai-images-v1'; modelField?: string }
...
/** One non-executing, redacted step shown before apply is authorized. */
export interface CanvasSkillConfigPreviewStep {
  kind: 'command' | 'file'
  /** JSON argv for commands; `<skill-config>/…` for files. */
  detail: string
  /** Full redacted file content. */
  content?: string
  status: 'ready' | 'skipped' | 'blocked'
  issue?: string
}

export interface CanvasSkillConfigPreviewRequest {
  name: string
  language?: string
}

export interface CanvasSkillConfigPreviewResult {
  ok: boolean
  /** False keeps the full preview visible but disables confirmation. */
  ready: boolean
  steps: CanvasSkillConfigPreviewStep[]
  /** Short-lived, one-time token returned only for a fully valid plan. */
...
export interface CanvasSkillConfigApplyRequest {
  name: string
  /** Token issued by configPreview after the user inspected the complete plan. */
  confirmationToken: string
  language?: string
}
