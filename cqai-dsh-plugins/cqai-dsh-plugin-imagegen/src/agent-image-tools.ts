...
export function ensureAgentImageConfigured(config: AgentImageToolConfig): void {
  if (!config.enabled) throw new ImageGenError('e图宝 is disabled. Open Settings > Plugins > e图宝 and enable it.', 'plugin-disabled')
  if (!config.allowAgentImageGeneration) throw new ImageGenError('Agent image generation is disabled in Settings > Plugins > e图宝.', 'agent-generation-disabled')
  const usable = config.channels.some(channel => channel.id === 'cqai' || (channel.apiUrl.trim() !== '' && channel.apiKey.trim() !== ''))
  if (!usable) throw new ImageGenError('Image API credentials are not configured. Open Settings > Plugins > e图宝, add a channel and fill in its API URL and API key.', 'image-api-not-configured')
}

/** Resolve a configured image alias and its owning channel. */
export function resolveAgentImageModel(config: AgentImageToolConfig, requested: unknown, provider?: unknown): { channel: RuntimeChannel; alias: string; upstream: string } {
  const requestedProvider = typeof provider === 'string' ? provider.trim() : ''
  const defaultProvider = config.channels.some(channel => channel.id === 'cqai') ? 'cqai' : config.defaultChannelId
  const providerId = requestedProvider === ''
    ? defaultProvider
    : requestedProvider === 'cqai' || requestedProvider.startsWith('custom:') ? requestedProvider : `custom:${requestedProvider}`
  const channels = providerId === '' ? config.channels : config.channels.filter(channel => channel.id === providerId)
  if (providerId !== '' && channels.length === 0) {
    throw new ImageGenError(`Image Provider "${providerId}" is not configured. CQAI failures do not fall back to a custom Provider.`, 'provider-not-configured')
  }
  const entries = channels.flatMap(channel => channel.models.map(model => ({ channel, alias: model.alias, upstream: model.id })))
  if (entries.length === 0) {
    throw new ImageGenError('No image models are configured. Open Settings > Plugins > e图宝 and add a channel with at least one model.', 'no-models-configured')
  }
  const wanted = typeof requested === 'string' && requested.trim() !== '' ? requested.trim() : ''
  if (wanted === '') {
    if (entries.length === 1) return entries[0]!
    const options = channels.flatMap(channel => channel.models.map(model => `"${channel.name} · ${model.alias}"`)).join(', ')
...
async function resolveAgentRequest(
  config: AgentImageToolConfig,
  request: GenerateRequest,
  provider: unknown,
  signal?: AbortSignal,
): Promise<GenerateRequest> {
  const requestedProvider = typeof provider === 'string' && provider.trim() !== '' ? provider.trim() : undefined
  const rawProviderId = requestedProvider ?? (config.channels.some(channel => channel.id === 'cqai') ? 'cqai' : config.defaultChannelId)
  const providerId = rawProviderId === 'cqai' || rawProviderId.startsWith('custom:') ? rawProviderId : `custom:${rawProviderId}`
  if (providerId === 'cqai' && config.resolveCqaiRequest !== undefined) {
    return config.resolveCqaiRequest({ ...request, channelId: 'cqai' }, signal)
  }
  const picked = resolveAgentImageModel(config, request.model, providerId)
  return {
    ...request,
    model: picked.alias,
...
/** Persist one completed queue task's generated images in DSH's attachment store. */
export function saveGenerationTaskImages(
  attachments: Pick<AttachmentStore, 'saveImages'>,
  task: GenerationTask,
): Promise<readonly ImageAttachmentRef[]> {
  if (task.status !== 'completed') return Promise.resolve([])
  return attachments.saveImages(
    (task.result?.images ?? []).map((image, index) => toSaveImage(image, task.id, index)),
  )
}
...
export function registerAgentImageTools(ctx: Context, runtime: ImageGenerationRuntime, resolve: () => AgentImageToolConfig): () => void {
  ...
  const materializeTaskImages = (task: GenerationTask): Promise<AgentImageRef[]> => {
    ...
    const pending = saveGenerationTaskImages(ctx.attachments, task)
