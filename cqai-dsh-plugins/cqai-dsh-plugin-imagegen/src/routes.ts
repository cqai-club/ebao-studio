...
export interface ImageGenRoutesDeps {
  ...
  /** Immutable CQAI Provider. Omission keeps the upstream standalone tests valid. */
  cqai?: Pick<CqaiImageProvider, 'describe' | 'resolveRequest' | 'complete' | 'listChatModels'>
  /** Remove secret values and replace them with Credentials-backed status bits. */
  projectSettingsDescriptor?: (descriptor: SettingsDescriptor) => SettingsDescriptor
  /** Partition secret ops into DSH Credentials before mutating public settings. */
  mutateSettings?: (ns: string, ops: readonly unknown[], expectedRevision?: number) => Promise<void>
  ...
  skillLibrary?: {
    ...
    /** Return the complete redacted plan and a one-time confirmation token. */
    configPreview?: (request: CanvasSkillConfigPreviewRequest) => Promise<CanvasSkillConfigPreviewResult>
    /** Run a declaration previously previewed and explicitly confirmed. */
...
export function makeRoutes(deps: ImageGenRoutesDeps): WebRoute[] {
  ...
  const projectDescriptor = (descriptor: SettingsDescriptor): SettingsDescriptor =>
    deps.projectSettingsDescriptor?.(descriptor) ?? descriptor
  ...
  const resolveChannelRequest = async (request: GenerateRequest): Promise<{ ok: true; request: GenerateRequest } | { ok: false; code: string; message: string }> => {
    // CQAI is the only implicit Provider. A custom channel must be named
    // explicitly and CQAI failures never cascade into third-party billing.
    const provider = request.channelId?.trim() ?? ''
    if (deps.cqai !== undefined && (provider === '' || provider === 'cqai')) {
      try {
        return { ok: true, request: await deps.cqai.resolveRequest(request) }
      } catch (error) {
        return {
          ok: false,
          code: error instanceof Error && 'code' in error && typeof (error as { code?: unknown }).code === 'string'
            ? (error as { code: string }).code
            : 'cqai-provider-failed',
    ...
    if (view.channels.length === 0) {
      return { ok: false, code: 'no-channels', message: '尚未配置任何渠道：请先在「设置 → 插件 → e图宝」添加渠道并填写 API 地址与密钥' }
    }
    const customProviderId = provider.startsWith('custom:') ? provider : `custom:${provider}`
    const explicit = view.channels.find(candidate => candidate.id === customProviderId)
    if (provider !== '' && explicit === undefined) {
      return { ok: false, code: 'provider-not-configured', message: `第三方 Provider「${provider}」未配置；CQAI 失败时不会自动切换渠道` }
    }
    ...
    const hosting = explicit === undefined
      ? view.channels.filter(channel => channel.models.some(model => model.alias === asked))
      : explicit.models.some(model => model.alias === asked) ? [explicit] : []
    if (hosting.length === 0) {
  ...
  return [
    ...
    // ----------------------------------------- immutable CQAI Provider view
    ...(deps.cqai === undefined ? [] : [{
      kind: 'exact' as const,
      path: CQAI_IMAGE_PROVIDER_API,
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        if (!guard(req, res, 'POST')) return
        const body = await readJsonBody(req)
        try {
          const provider: CqaiImageProviderView = await deps.cqai!.describe({
    ...
    //   fallback, and the body's apiUrl/apiKey override them for this call.
    {
      ...
      handler: async (req, res) => {
        if (!guard(req, res, 'POST')) return
        const body = await readJsonBody(req)
        if (deps.cqai !== undefined && (body?.channelId === undefined || body.channelId === 'cqai')) {
          try {
            const provider = await deps.cqai.describe({ refresh: true })
            writeJson(res, 200, { ok: true, models: provider.models.map(model => model.alias) })
          } catch (error) {
    ...
    // ----------------------------------------------- prompt enhancement
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          writeJson(res, 200, {
            ok: true,
            models: deps.cqai === undefined
              ? await listPromptModels(resolvePrompt())
              : await deps.cqai.listChatModels(),
          })
        } catch (error) {
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const enhanced = deps.cqai === undefined
            ? await enhancePrompt(resolvePrompt(), prompt)
          ...
          writeJson(res, 200, { ok: true, prompt: enhanced })
        } catch (error) {
    ...
    // ----------------------------------------------------------- generate
    {
      ...
      handler: async (req, res) => {
        ...
        const resolved = await resolveChannelRequest(parsed)
        if (!resolved.ok) {
    ...
    // model (the prompt-enhancement endpoint) for a strict JSON layer plan.
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const plan = deps.cqai === undefined
            ? await analyzeLayers(resolvePrompt(), raw)
            : await analyzeLayersWithChat(options => deps.cqai!.complete(options), raw)
    ...
    {
      kind: 'exact',
      path: CANVAS_SKILL_API.configPreview,
      handler: async (req, res) => {
        if (!guard(req, res, 'POST')) return
        const body = await readJsonBody(req)
        if (deps.skillLibrary?.configPreview === undefined) {
          writeJson(res, 200, { ok: false, code: 'library-unavailable', message: '本宿主不支持预览技能配置操作。', steps: [] })
          return
        }
        const name = typeof body?.name === 'string' ? body.name.trim() : ''
        if (name === '') {
          writeJson(res, 200, { ok: false, code: 'bad-request', message: '缺少技能名。', steps: [] })
          return
        }
        try {
          writeJson(res, 200, await deps.skillLibrary.configPreview({ name, ...parseSkillLanguage(body) }))
        } catch (error) {
          writeJson(res, 200, { ok: false, code: 'config-preview-failed', message: messageOf(error), steps: [] })
        }
      },
    },
    {
      ...
      handler: async (req, res) => {
        ...
        if (confirmationToken === '') {
          writeJson(res, 200, { ok: false, code: 'confirmation-required', message: '请先预览全部操作并明确确认。', steps: [] })
          return
        }
        try {
          writeJson(res, 200, await deps.skillLibrary.configApply({ name, confirmationToken, ...parseSkillLanguage(body) }))
        } catch (error) {
