/**
 * Standalone smoke test for the built cqai-dsh-plugin-imagegen artifacts:
 *
...
await check('A2 Config schema validates + marks apiKey secret', () => {
  ...
  assert.equal(host.Config.dict?.storageAccessKey?.meta?.role, 'secret')
  assert.equal(host.Config.dict?.storageSecretKey?.meta?.role, 'secret')
})
  ...
  if (url.pathname === '/v1/images/generations') {
    ...
    const data = body.prompt === 'foreign url'
      ...
      : [
          ...
          // Happy-path URL normalization uses a self-contained data URL. The
          // dedicated checks below keep loopback HTTP results fenced off, as
          // production now requires generated HTTP(S) URLs to resolve only to
          // public addresses before opening a socket.
          { url: pngDataUrl },
        ]
...
await check('B1 text generation normalizes b64_json + data-url items', async () => {
  ...
  assert.equal(resultAuthHeaders.length, 0, 'data URLs never open a result-download socket')
})

await check('B2 non-public signed result URLs are blocked before credentials or network', async () => {
  ...
  assert.equal(resultAuthHeaders.length, 0, 'blocked signed URLs never reach the local result handler')
})

await check('B2b foreign non-public result URLs are blocked before auth or network', async () => {
  await assert.rejects(
    host.generateImage(
      { apiUrl: `http://127.0.0.1:${upstreamPort}/v1`, apiKey: 'sk-test' },
  ...
  assert.equal(foreignAuthHeaders.length, 0, 'blocked foreign URLs never receive the Bearer key')
})
...
await check('B7 Volcengine Seedream uses Ark size and URL response fields', async () => {
  ...
  const seedream = createServer(async (req, res) => {
    ...
    if (url.pathname === '/v1/images/generations') {
      ...
      res.writeHead(200, { 'content-type': 'application/json' })
      res.end(JSON.stringify({ data: [{ url: pngDataUrl }] }))
      return
    }
  ...
  const qwen = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/v1/services/aigc/multimodal-generation/generation') {
      ...
      res.end(JSON.stringify({
        output: { choices: [{ message: { content: [{ image: pngDataUrl }] } }] },
        usage: { image_count: 1 },
      }))
      return
    }
  ...
  const asyncProvider = createServer(async (req, res) => {
    ...
    if (url.pathname.startsWith('/v1/tasks/')) {
      ...
      if (count === 1) {
        ...
        res.end(JSON.stringify({ data: { status: 'completed', task_id: taskId, result: { images: [{ url: [pngDataUrl, pngDataUrl] }] } } }))
      }
      return
    }
    res.writeHead(404)
...
await check('C9 Agent tools wait for results, keep images in the UI view, edit, and enforce the allow setting', async () => {
  ...
  const cqaiChannel = {
    ...
    request: async (path, init, signal) => {
      ...
      return fetch(`http://127.0.0.1:${upstreamPort}${path}`, { ...init, headers, signal })
    },
  }
  const cqaiConfig = allow => ({
    enabled: true,
    allowAgentImageGeneration: allow,
    defaultChannelId: 'cqai',
    channels: [cqaiChannel],
    async resolveCqaiRequest(request) {
      cqaiResolutions += 1
      const model = request.model.trim() === '' ? 'gpt-image-2' : request.model.trim()
      if (model !== 'gpt-image-2') throw new Error(`Image model "${model}" is not available for CQAI`)
      return { ...request, model, upstream: model, channelId: 'cqai', channel: 'CQAI' }
    },
  })
  ...
  try {
    ...
    assert.equal(runtime.queue.list().find(task => task.id === generated.task_id)?.request.channelId, 'cqai', 'omitting provider uses CQAI')
    ...
    const commandDispose = host.registerEditImageCommand({
      ...
      attachments: attachmentStore,
    }, runtime, () => cqaiConfig(true))
    try {
      assert.ok(commands.has('edit_image'), 'the /edit_image command is registered')
      const command = commands.get('edit_image')
      assert.deepEqual(command.input, { hint: 'Describe how to modify the latest image', attachments: true }, 'the native command attachment contract is enabled')
    ...
    assert.ok(cqaiResolutions >= 4, 'generation, background work, editing and the command all resolve through CQAI')
  } finally {
    dispose()
  }
})
...
await check('C13a a skill.config.json is validated, bounded and versioned', () => {
  ...
  const compatible = host.parseSkillConfigManifest({
    version: 1,
    fields: [{ id: 'image-model', type: 'string' }],
    imageProvider: { protocol: 'openai-images-v1', modelField: 'image-model' },
  })
  assert.deepEqual(compatible.manifest.imageProvider, {
    protocol: 'openai-images-v1', modelField: 'image-model',
  })
  assert.equal(host.parseSkillConfigManifest({
    version: 1,
    imageProvider: { protocol: 'some-other-api' },
  }).issue, 'empty')
  ...
  // File targets are confined to a host-owned per-skill root: relative paths
  // resolve inside it, while home expansion, absolute paths and traversal are
  // all refused before preview or apply.
  const configRoot = join(skillRunRoot, 'private-config')
  assert.deepEqual(host.resolveConfigTarget('editppt/config.yaml', configRoot), {
    path: join(configRoot, 'editppt', 'config.yaml'),
    relative: join('editppt', 'config.yaml'),
  })
  assert.equal(host.resolveConfigTarget('~/.editppt/config.yaml', configRoot).issue, 'refused-path')
  assert.equal(host.resolveConfigTarget('/tmp/outside/config.yaml', configRoot).issue, 'refused-path')
  assert.equal(host.resolveConfigTarget('../outside/config.yaml', configRoot).issue, 'refused-path')
})
...
await check('C13b apply steps substitute values, honor guards and mask secrets', async () => {
  ...
  try {
    const parsed = host.parseSkillConfigManifest({
      ...
      apply: [
        { kind: 'command', argv: [process.execPath, '-e', 'console.log("endpoint " + process.argv[1])', '{base-url}'] },
        { kind: 'command', argv: [process.execPath, '-e', `require('node:fs').writeFileSync(${JSON.stringify(join(root, 'secret-command-ran'))}, process.argv[1])`, '{api-key}'] },
        { kind: 'file', path: 'out/config.json', content: '{"base":"{base-url}","key":"{api-key}"}' },
    ...
    const configRoot = join(root, 'private-config')
    const preview = await host.previewSkillConfigSteps(declaration, values, { configRoot })
    assert.equal(preview.ready, false, 'a secret-bearing argv blocks the whole preview')
    assert.equal(preview.steps[1].status, 'blocked')
    assert.equal(preview.steps[1].detail.includes('sk-super-secret'), false)
    assert.match(preview.steps[1].detail, /•••/)
    assert.equal(preview.steps[2].content.includes('sk-super-secret'), false)
    assert.match(preview.steps[2].content, /•••/)

    const steps = await host.applySkillConfigSteps(declaration, values, { runRoot: root, configRoot })
    assert.equal(steps.length, 4)
    // Public arguments may run normally.
    assert.equal(steps[0].ok, true)
    assert.match(steps[0].output, /endpoint https:\/\/x\/v1/)
    // Secret placeholders are rejected before spawning a process and never
    // appear in the result sent back to the panel.
    assert.equal(steps[1].ok, false)
    assert.equal(steps[1].detail.includes('sk-super-secret'), false)
    assert.equal(steps[1].output.includes('sk-super-secret'), false)
    assert.match(steps[1].detail, /•••/)
    assert.match(steps[1].output, /不能作为命令行参数/)
    assert.throws(() => readFileSync(join(root, 'secret-command-ran')), /ENOENT/)
    // Secrets may be written only into the private per-skill root.
    assert.equal(steps[2].ok, true)
    assert.equal(steps[0].detail.includes('sk-super-secret'), false)
    assert.equal(readFileSync(join(configRoot, 'out', 'config.json'), 'utf8'), '{"base":"https://x/v1","key":"sk-super-secret"}')
    // The guarded step is skipped: its field has no value.
    assert.equal(steps[3].detail, 'skipped')

    // A missing required value stops that step with copy, not a crash.
    const empty = host.valuesFor(declaration, { values: {}, secrets: {} }, 'deck')
    const failed = await host.applySkillConfigSteps(declaration, empty, { runRoot: root, configRoot })
    assert.equal(failed[0].ok, false)
    assert.match(failed[0].output, /base-url/)
  } finally {
...
await check('C13c the built-in recipe covers image-to-editable-ppt and a sidecar wins', async () => {
  ...
  try {
    ...
    assert.deepEqual(recipe.declaration.manifest.fields.map(field => field.id), ['paddle-ocr-token'])
    assert.deepEqual(recipe.declaration.manifest.imageProvider, { protocol: 'openai-images-v1' })
    assert.deepEqual(recipe.view.imageProvider, { protocol: 'openai-images-v1' })
    assert.deepEqual(recipe.view.missing, [])
    assert.equal(recipe.view.applicable, true)
    // The optional OCR setup remains persistent in the skill-private config
    // root; image access itself is run-scoped and no secret enters argv.
    assert.equal(recipe.declaration.manifest.steps[0].kind, 'file')
    assert.equal(recipe.declaration.manifest.steps[0].path, 'editppt/config.yaml')
    assert.match(recipe.declaration.manifest.steps[0].content, /\{paddle-ocr-token\}/)
    assert.deepEqual(recipe.declaration.manifest.steps[0].when, { field: 'paddle-ocr-token', set: true })
    ...
    writeFileSync(join(root, 'image-to-editable-ppt', 'skill.config.json'), JSON.stringify({
      ...
      apply: [{ kind: 'file', path: 'own.txt', content: '{own-field}' }],
    }))
...
await check('C13d the config routes save and apply through the library backend', async () => {
  const seen = {}
  const previous = { save: skillLibraryFake.configSave, preview: skillLibraryFake.configPreview, apply: skillLibraryFake.configApply }
  skillLibraryFake.configSave = async request => {
    seen.saved = request
    return { ok: true, library: await skillLibraryFake.list() }
  }
  skillLibraryFake.configPreview = async request => {
    seen.previewed = request
    return {
      ok: true,
      library: await skillLibraryFake.list(),
      ready: true,
      confirmationToken: 'smoke-confirmation-token',
      expiresAt: Date.now() + 60_000,
      steps: [{ kind: 'file', detail: '<skill-config>/editppt/config.yaml', content: 'PADDLE_OCR_TOKEN: •••', status: 'ready' }],
    }
  }
  skillLibraryFake.configApply = async request => {
    seen.applied = request
    assert.equal(request.confirmationToken, 'smoke-confirmation-token', 'only the preview-bound token reaches apply')
    return { ok: true, library: await skillLibraryFake.list(), steps: [{ kind: 'file', detail: '→ <skill-config>/editppt/config.yaml', ok: true }] }
  }
  try {
    const saved = await post('/api/dsh-imagegen/canvas/skills/config/save', {
      name: 'image-to-editable-ppt',
      values: [{ id: 'paddle-ocr-token', value: 'ocr-1' }, { bad: true }, { id: 'x' }],
      language: 'zh',
    })
    assert.equal(saved.body.ok, true, JSON.stringify(saved.body))
    assert.deepEqual(seen.saved, { name: 'image-to-editable-ppt', values: [{ id: 'paddle-ocr-token', value: 'ocr-1' }], language: 'zh' })

    const unconfirmed = await post('/api/dsh-imagegen/canvas/skills/config/apply', { name: 'image-to-editable-ppt', language: 'zh' })
    assert.equal(unconfirmed.body.ok, false)
    assert.equal(unconfirmed.body.code, 'confirmation-required')
    assert.equal(seen.applied, undefined, 'an unconfirmed request never reaches the backend apply seam')

    const previewed = await post('/api/dsh-imagegen/canvas/skills/config/preview', { name: 'image-to-editable-ppt', language: 'zh' })
    assert.equal(previewed.body.ok, true, JSON.stringify(previewed.body))
    assert.equal(previewed.body.ready, true)
    assert.equal(previewed.body.confirmationToken, 'smoke-confirmation-token')
    assert.deepEqual(seen.previewed, { name: 'image-to-editable-ppt', language: 'zh' })

    const applied = await post('/api/dsh-imagegen/canvas/skills/config/apply', {
      name: 'image-to-editable-ppt',
      confirmationToken: previewed.body.confirmationToken,
      language: 'zh',
    })
    assert.equal(applied.body.ok, true, JSON.stringify(applied.body))
    assert.equal(applied.body.steps[0].kind, 'file')
    assert.deepEqual(seen.applied, { name: 'image-to-editable-ppt', confirmationToken: 'smoke-confirmation-token', language: 'zh' })
    ...
    skillLibraryFake.configPreview = previous.preview
...
await check('D1 client bundle registers via __ModuleLoader__ and exposes the canvas raster helpers', () => {
  ...
  assert.equal(handoff.id, 'cqai-dsh-plugin-imagegen')
  ...
  assert.equal([...exportsOf.inject].join(','), 'slots,locale,connection,conversation')
...
// --------------- E. full client apply in jsdom (official panel registration)
await check('E1 client apply registers and renders the official image studio (jsdom)', async () => {
  const { JSDOM } = await import('jsdom')
  const dom = new JSDOM(
    '<!doctype html><html lang="zh-CN"><head></head><body><div data-imagegen-smoke-root></div></body></html>',
  ...
  const configState = {
    ...
    defaultChannelId: 'custom:default',
  }
  ...
  const fetchStub = async (input, init) => {
    ...
    if (path.endsWith('/settings/mutate')) {
      ...
      for (const op of payload.ops) {
        ...
        if (op.path[0] === 'channelSecrets' && op.path[1] === 'custom:default') keyState.set = op.op === 'set'
      }
      return {
        ok: true,
        json: async () => ({
  ...
  const connection = { isLoopback: true, rpc: async () => ({ ok: true }) }
  const conversation = {
    createDrafts: () => [],
    ...
    slots: {
      inject(key, callback) {
        ...
        if (result !== null && typeof result === 'object' && Symbol.iterator in result) {
          for (const _dispose of result) { /* exhaust Cordis registration generator */ }
        }
        return () => {}
      },
  ...
  exportsOf.apply(ctx)
  const mainEntry = registered.find(entry => entry.options.name === 'main' && entry.options.key === 'cqai-imagegen')
  const studioEntry = registered.find(entry => entry.options.name === 'cqai-imagegen.session')
  const sidebarEntry = registered.find(entry => entry.options.name === 'sidebar.panellist' && entry.options.id === 'cqai-imagegen')
  assert.ok(mainEntry !== undefined, 'official keyed main entry registered')
  assert.ok(studioEntry !== undefined, 'session-maybe studio seat registered')
  assert.ok(sidebarEntry !== undefined, 'official sidebar panel entry registered')
  assert.equal(sidebarEntry.options.label(), 'e图宝', 'sidebar uses the e图宝 product name')
  assert.equal(JSON.stringify(mainEntry.options.children), JSON.stringify({
    'cqai-imagegen.session': { kind: 'single', scope: 'session-maybe' },
  }))

  const studioHost = jsdomDocument.querySelector('[data-imagegen-smoke-root]')
  const studioRoot = (await import('react-dom/client')).createRoot(studioHost)
  studioRoot.render(react.createElement(mainEntry.component, {
    renderSlot: (name) => {
      assert.equal(name, 'cqai-imagegen.session')
      return react.createElement(studioEntry.component, {
        sessionId: undefined,
  ...
  try {
    ...
    const normalConfig = view.querySelector('[data-dsh-imagegen-normal-config]')
    const normalFeed = view.querySelector('[data-dsh-imagegen-normal-feed]')
    assert.ok(normalConfig !== null, 'ordinary generation config is rendered')
    assert.ok(normalFeed !== null, 'ordinary generation feed is rendered')
    assert.equal(normalFeed.getAttribute('data-layout'), 'masonry', 'ordinary generation feed declares its masonry layout')
    assert.ok(
      (normalConfig.compareDocumentPosition(normalFeed) & jsdomWindow.Node.DOCUMENT_POSITION_FOLLOWING) !== 0,
    ...
    assert.equal(view.querySelector('[data-dsh-imagegen-history]'), null, 'asset library does not render the legacy history rail')
    ...
    const settingsRegistration = registered.find(entry => entry.options.name === 'settings.plugin.item' && entry.options.key === 'dsh-imagegen')
    assert.ok(settingsRegistration !== undefined, 'image settings card registered')

    // Neither the asset library nor ordinary generation renders the retired
    // inspiration wall. The explicit template library remains available.
    assert.equal(jsdomDocument.querySelector('[aria-label="灵感案例"]'), null, 'asset library hides inspiration wall')
    normalSwitch.click()
    await new Promise(resolve => setTimeout(resolve, 50))
    assert.ok(view.querySelector('[data-dsh-imagegen-normal-feed]') !== null, 'ordinary task feed returns after leaving the gallery')
    assert.equal(jsdomDocument.querySelector('[aria-label="灵感案例"]'), null, 'ordinary generation keeps inspiration examples hidden')
    ...
    const face = settingsRegistration.options.inject()
    face.channels.setChannelKey('custom:default', 'sk-new')
    ...
    assert.ok(
      mutateCalls.some(m => m.ops.some(o => o.path[0] === 'channelSecrets' && o.path[1] === 'custom:default' && o.op === 'set' && o.value === 'sk-new')),
      'channel key write sent with the typed value',
    )

    // Ecommerce submission flow: the signed-in CQAI default fans out one
    // queued task per planned image carrying the set metadata.
    ...
    // also report success.
    face.channels.setChannelKey('custom:default', '')
    await face.channels.commit()
    await new Promise(resolve => setTimeout(resolve, 100))
    ...
    assert.equal(face.hooks.imageGenSettingsCard.getSnapshot().channels.keySet['custom:default'], false, 'key-set flag follows the clear')
    canvasSwitch.click()
    ...
    assert.equal(view.querySelector('[data-dsh-imagegen-history]'), null, 'infinite canvas does not render the legacy history rail')
    ...
    // Generate from the config node: the request must carry the boxed
    // reference. Clearing an unrelated custom key must not disconnect CQAI.
    ...
    // the browser), so assert the CQAI-backed composer remains enabled.
    assert.ok(await waitUntil(() => view.querySelector('[aria-label="生成图片"]')?.disabled === false), 'composer remains enabled through CQAI')
