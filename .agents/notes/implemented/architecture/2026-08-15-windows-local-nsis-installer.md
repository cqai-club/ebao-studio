...
Electron Builder's existing `afterPack` hook verifies the application archive, physical runtime tree, and required Windows x64 `node-pty` prebuilds before NSIS packaging. After packaging, a second gate requires the exact versioned installer and `win-unpacked/易宝工坊.exe`, verifies that both are non-empty regular files, and checks their DOS `MZ` and Windows `PE\0\0` signatures. A stale installer from another version cannot satisfy the gate.
