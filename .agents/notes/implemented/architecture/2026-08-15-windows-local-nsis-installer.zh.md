...
Electron Builder 现有的 `afterPack` hook 会在 NSIS 封装前校验 application archive、物理 runtime tree 与必需的 Windows x64 `node-pty` prebuild。打包完成后，第二个 gate 会要求准确版本的安装器与 `win-unpacked/易宝工坊.exe` 存在，验证两者都是非空普通文件，并检查其 DOS `MZ` 与 Windows `PE\0\0` signature。其他版本遗留的安装包无法通过该 gate。
